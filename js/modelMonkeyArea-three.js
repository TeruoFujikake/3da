...
const camera = new THREE.PerspectiveCamera(
  75,
  model3dAreaWidth / model3dAreaHeight,
  0.1,
  1000
);
...
let modelTop, modelCenter, modelBottom;

// 上：see-no-evil_monkey
loader.load("model/monkey/see-no-evil_monkey/scene.gltf", function (gltf) {
  modelTop = gltf.scene;

  // モデルのスケールと位置を調整
  modelTop.scale.set(3, 3, 3);
  modelTop.position.set(0, 4, 0); // 上に配置（y=4など、間隔は調整可）
...
// 中央：speak-no-evil_monkey
loader.load("model/monkey/speak-no-evil_monkey/scene.gltf", function (gltf) {
  modelCenter = gltf.scene;
  modelCenter.scale.set(3, 3, 3);
  modelCenter.position.set(0, 0, 0); // 中央

  modelCenter.traverse((child) => {
    if (child.isMesh) {
  ...
  scene.add(modelCenter);
});

// 下：hear-no-evil_monkey
loader.load("model/monkey/hear-no-evil_monkey/scene.gltf", function (gltf) {
  modelBottom = gltf.scene;
  modelBottom.scale.set(3, 3, 3);
  modelBottom.position.set(0, -4, 0); // 下に配置（y=-4など、間隔は調整可）
...
function animate() {
  ...
  if (modelCenter) modelCenter.rotation.y -= 0.005;
